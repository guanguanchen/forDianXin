...
public class MediumTwo implements ActionListener, Runnable{
	JLabel label;
	//MainWindow.main_windowWidth = 256 + 160;
	//MainWindow.main_windowLength = 298 + 80 + 160;
	...
	public void run() {
		...
		while(MainWindow.main_window.medium_choose){
	...
	*/
	...
	public void createManyButton(){
		...
		for(int i = 0; i < 24; i++){
			...
			}else if(block_be_chosen_random.get(i) < 10){
				j = rand.nextInt(random_to_button.size());
				createButton(80 * (temp - 5), 160, temp, random_to_button.get(j));
				random_to_button.remove(j);
			}else if(block_be_chosen_random.get(i) < 15){
				j = rand.nextInt(random_to_button.size());
				createButton(80 * (temp - 10), 240, temp, random_to_button.get(j));
				random_to_button.remove(j);
			}else if(block_be_chosen_random.get(i) < 20) {
				j = rand.nextInt(random_to_button.size());
				createButton(80 * (temp - 15), 320, temp, random_to_button.get(j));
				random_to_button.remove(j);
			}else if(block_be_chosen_random.get(i) < 25) {
				j = rand.nextInt(random_to_button.size());
				createButton(80 * (temp - 20), 400, temp, random_to_button.get(j));
				random_to_button.remove(j);
			}
	...
	public void ButtonMove(int num_click,int block_identifier) { //up
		...
		if(block_identifier > 4) {
			if(block_to_button.get(block_identifier - 5) == null) {
				...
				block_to_button.replace(block_identifier - 5, block_to_button.get(block_identifier));
				block_to_button.replace(block_identifier, null);
				num_to_block.replace(num_click, block_identifier - 5);
				isWin();
				return;
			}
		}if(block_identifier < 20) { //down
			if(block_to_button.get(block_identifier + 5) == null) {
				block_to_button.get(block_identifier).setLocation(block_to_button.get(block_identifier).getX(), block_to_button.get(block_identifier).getY() + 80);
				block_to_button.replace(block_identifier + 5, block_to_button.get(block_identifier));
				block_to_button.replace(block_identifier, null);
		...
		}if(block_identifier != 0 && block_identifier != 5 && block_identifier != 10 && block_identifier != 15 && block_identifier != 20) { //left
		...
		}if(block_identifier != 4 && block_identifier != 9 && block_identifier != 14 && block_identifier != 19 && block_identifier != 24) { //right
	...
	public void isWin() {
		int i;
		for(i = 0; i < 24 && num_to_block.get(i + 1) == i; i++){
